# Aerospike ESP Outbound custom code plugin example

This example deploys Aerospike ESP Outbound connectors with custom code plugin and an Aerospike cluster without TLS configured. 
...
## Prerequisites
 ...
 - Aerospike ESP Outbound Connector [Helm chart](../../README.md#adding-the-helm-chart-repository)
...
Update the [as-esp-outbound-values.yaml](as-esp-outbound-values.yaml) file to change ESP connector configuration to point to backend ESP endpoint. 
Also, add the initContainer section to the file to use your custom code plugin image.

Deploy the connectors using configuration from [as-esp-outbound-values.yaml](as-esp-outbound-values.yaml)
...
```shell
helm -n aerospike uninstall as-esp-outbound
```
