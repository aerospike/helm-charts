...
if [ -f "$LOCAL_FEATURES_CONF" ]; then
...
elif [ -f "$WORKSPACE/aerospike-jms-inbound/kind/config/features.conf" ]; then
  echo "Using features.conf from workspace: $WORKSPACE/aerospike-jms-inbound/kind/config/features.conf"
  FEATURES_CONF="$WORKSPACE/aerospike-jms-inbound/kind/config/features.conf"
else
  ...
  echo "  - $WORKSPACE/aerospike-jms-inbound/kind/config/features.conf (workspace)"
  exit 1
fi

echo "Installing Kind"
kind create cluster --config "$WORKSPACE/aerospike-jms-inbound/kind/config/kind-cluster.yaml"
kubectl cluster-info --context kind-jms-inbound-test-cluster
...
kubectl create clusterrolebinding aerospike-cluster-jms-inbound-test \
...
echo "2. Or deploy JMS Inbound manually: helm install test-jms-inbound ../aerospike-jms-inbound --namespace aerospike-test"
echo ""
