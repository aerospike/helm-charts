...
mkdir -p "$WORKSPACE/aerospike-proximus/proximus-examples/kind/data"

curl -L -o "$WORKSPACE/aerospike-proximus/proximus-examples/kind/data/quotes.csv.tgz" \
...
docker run -d \
--name "quote-search" \
-v "$WORKSPACE/aerospike-proximus/kind/data:/container-volumes/quote-search/data" \
