...
# Test script for aerospike-jms-outbound Helm chart
...
CLEAR_TEXT_VALUES="examples/clear-text/as-jms-outbound-values.yaml"
TLS_VALUES="examples/tls/as-jms-outbound-tls-values.yaml"

echo "🧪 Testing Aerospike JMS Outbound Helm Chart"
echo "=============================================="
...
if command -v kubectl &> /dev/null; then
    ...
    kubectl wait --for=condition=ready pod -l app.kubernetes.io/name=aerospike-jms-outbound --namespace $NAMESPACE --timeout=60s
    ...
    kubectl run test-pod --image=busybox --rm -it --restart=Never --namespace $NAMESPACE -- sh -c "nslookup $RELEASE_NAME-aerospike-jms-outbound; sleep 5" || true
