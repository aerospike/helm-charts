...
# Test script for aerospike-kafka-outbound Helm chart

set -e

CHART_NAME="."
RELEASE_NAME="test-kafka-outbound"
NAMESPACE="aerospike-test"
CLEAR_TEXT_VALUES="examples/clear-text/as-kafka-outbound-values.yaml"
TLS_VALUES="examples/tls/as-kafka-outbound-tls-values.yaml"

echo "🧪 Testing Aerospike Kafka Outbound Helm Chart"
echo "=============================================="
...
if command -v kubectl &> /dev/null; then
    ...
    kubectl wait --for=condition=ready pod -l app.kubernetes.io/name=aerospike-kafka-outbound --namespace $NAMESPACE --timeout=60s
    ...
    kubectl run test-pod --image=busybox --rm -it --restart=Never --namespace $NAMESPACE -- sh -c "nslookup $RELEASE_NAME-aerospike-kafka-outbound; sleep 5" || true
