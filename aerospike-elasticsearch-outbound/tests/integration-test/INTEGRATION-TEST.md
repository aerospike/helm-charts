...
helm uninstall test-elasticsearch-outbound xdr-proxy -n aerospike-test 2>&1 | grep -v "not found" || true
...
2. Verify ESP pods are accessible:
   ```bash
   kubectl exec -n aerospike-test $SRC_POD -- nc -zv test-elasticsearch-outbound-aerospike-elasticsearch-outbound-0.test-elasticsearch-outbound-aerospike-elasticsearch-outbound 8901
   ```
...
2. Verify XDR Proxy service is accessible:
   ```bash
   kubectl exec -n aerospike-test test-elasticsearch-outbound-aerospike-elasticsearch-outbound-0 -- nc -zv xdr-proxy-aerospike-xdr-proxy.aerospike-test.svc.cluster.local 8901
   ```
