...
# Test script for aerospike-pulsar-outbound Helm chart

set -e

CHART_NAME="."
RELEASE_NAME="test-pulsar-outbound"
NAMESPACE="aerospike-test"
CLEAR_TEXT_VALUES="examples/clear-text/as-pulsar-outbound-values.yaml"
TLS_VALUES="examples/tls/as-pulsar-outbound-tls-values.yaml"

echo "🧪 Testing Aerospike Pulsar Outbound Helm Chart"
echo "=============================================="
...
if command -v kubectl &> /dev/null; then
    ...
    kubectl wait --for=condition=ready pod -l app.kubernetes.io/name=aerospike-pulsar-outbound --namespace $NAMESPACE --timeout=60s
    ...
    kubectl run test-pod --image=busybox --rm -it --restart=Never --namespace $NAMESPACE -- sh -c "nslookup $RELEASE_NAME-aerospike-pulsar-outbound; sleep 5" || true
